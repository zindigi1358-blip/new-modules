...
def get_remediation(exposure_flags: list, technology: str, cves: list) -> dict:
    ...
    for flag in (exposure_flags or []):
        if flag in REMEDIATIONS:
            return REMEDIATIONS[flag]

    # Generic CVE remediation
    if cves:
        return {
            "title": f"Update {technology} to latest stable version",
            "priority": "HIGH — patch within 1 week",
            "description": (
                f"The detected version of {technology} is affected by known CVEs. "
            ...
            "steps": [
                ...
                "Re-scan after patching to confirm remediation.",
            ],
            "code": {},
        }
...
def build_executive_summary(styles, data: dict) -> list:
    ...
    total_findings  = m3.get("total_findings", 0)
    critical_count  = m3.get("critical_count", 0)
    high_count      = m3.get("high_count", 0)
    medium_count    = m3.get("medium_count", 0)
    low_count       = m3.get("low_count", 0)
    kev_matches     = m3.get("kev_matches", 0)
    leak_findings   = len(leaks.get("findings", []))
...
def build_risk_overview(styles, data: dict) -> list:
    ...
    sev_tbl = Table(table_data, colWidths=[col_w * 0.14, col_w * 0.08,
                                            col_w * 0.28, col_w * 0.50])
...
def build_technical_findings(styles, data: dict) -> list:
    ...
    for lf in leak_f:
        if lf.get("risk", "").upper() in ("CRITICAL", "HIGH"):
            findings.append({
                "subdomain":    lf.get("subdomain", lf.get("url", "?")),
                "technology":   "File Exposure",
                "version":      None,
                "risk_level":   lf.get("risk", "HIGH").upper(),
                "risk_score":   90 if lf.get("risk") == "critical" else 70,
                "matched_cves": [],
                "in_kev":       False,
...
def build_appendix(styles, data: dict) -> list:
    ...
    sorted_cves = sorted(all_cves.values(), key=lambda x: x.get("cvss", 0), reverse=True)
